package com.whitecrow.team.service.impl;
...
import static com.whitecrow.constant.RedissonConstant.JOIN_TEAM_LOCK;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team>
    ...
    public long addTeam(Team team, User loginUser) {
        ...
        if (maxNum < 1 || maxNum > 20) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "队伍人数不满足");
        }
        //     *    2. 队伍标题 <= 20
        ...
        if (StringUtils.isNotBlank(description) && description.length() > 512) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "描述超过限制");
        }
        //     *    4. status 是否公开（int）不传默认为 0（公开）
        ...
        if (new Date().after(expireTime)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "组队超时");
        }
        //     *    7. 校验用户最多创建 5 个队伍
    ...
    public List<TeamUserVO> listTeam(TeamQuery teamQuery, boolean isAdmin) {
        ...
        if (teamQuery != null) {
            ...
            if (!isAdmin && TeamStatusEnum.PRIVATE.equals(TeamStatusEnum.getEnumByValue(status))) {
    ...
    private List<UserVO> getUserTeamList(Long teamId){
        if(teamId==null||teamId<=0){
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
