package com.whitecrow.user.service.impl;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 盐值，混淆密码
     */
    private static final String SALT = "whitecrow";
    ...
    public long userRegister(UserRegisterRequest user) {
        ...
        userReal.setUserAccount(userAccount);
        userReal.setUserPassword(encryptPassword);
        userReal.setGender(user.getGender());
        userReal.setEmail(user.getEmail());
        userReal.setTag(user.getTag());
        userReal.setAvatarUrl(user.getAvatarUrl());
        userReal.setPhone(user.getPhone());
        userReal.setUsername(user.getUsername());
        boolean saveResult = this.save(userReal);
        if (!saveResult) {
            return -1;
        }
        return userReal.getId();
    }
    ...
    public User userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        log.info("a_________________________");
        return safetyUser;
    }
    ...
    @Override
    public Boolean isAdmin(User userLoginUser) {
        return userLoginUser != null && userLoginUser.getUserRole() == ADMIN_ROLE;
    }
    ...
    public List<User> matchUsers(long num, User loginUser) {
        ...
        for (User user : userList) {
            ...
            // 无标签或者为当前用户自己
            if (StringUtils.isBlank(userTag) || Objects.equals(user.getId(), loginUser.getId())) {
                continue;
            }
        ...
        LinkedList<User> finalUserList = new LinkedList<>();
        for (Long userId : userIdList) {
            finalUserList.addLast(userIdUserListMap.get(userId).get(0));
        }
